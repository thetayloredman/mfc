...
func (c *Client) QueryForKeyDirectly(serverName string) (KeyResponse, error) {
	...
	if err != nil {
		return KeyResponse{}, fmt.Errorf("failed to query for key directly from server %s: %v", serverName, err)
	}

	if respCode != 200 {
		return KeyResponse{}, fmt.Errorf("failed to query for key directly from server %s: got response code %d", serverName, respCode)
	}

	var keyResponse KeyResponse
	untypedResponse, err := json.Marshal(response)
	if err != nil {
		return KeyResponse{}, fmt.Errorf("failed to marshal response from server %s: %v", serverName, err)
	}

	err = json.Unmarshal(untypedResponse, &keyResponse)
	if err != nil {
		return KeyResponse{}, fmt.Errorf("failed to unmarshal response from server %s: %v", serverName, err)
	}
	...
	fetcher := NewProvidedKeyFetcher(keyResponse.ServerName, knownKeys)

	verificationError := jsonsigning.VerifyJSON(response, keyResponse.ServerName, fetcher)

	if verificationError != nil {
		return KeyResponse{}, fmt.Errorf("failed to verify response from server %s: %v", serverName, verificationError)
	}
